...
// Палитра красивых цветов для пингов
const PING_COLORS = [
...
// Функция, которая всегда выдает один и тот же цвет для одного ID
const getUserColor = (accountId) => {
...
const SERVER_URL = 'http://45.131.186.23:3000';
const socket = io(SERVER_URL);

// Звук броска кубиков
const rollSound = typeof Audio !== 'undefined' ? new Audio('/sounds/dice-roll.mp3') : null;
const DELETE_FIELD = '__DELETE_FIELD__';

const segmentIntersection = (x1, y1, x2, y2, x3, y3, x4, y4) => {
    const det = (x2 - x1) * (y4 - y3) - (x4 - x3) * (y2 - y1);
    if (Math.abs(det) < 1e-10) return null; // параллельны или совпадают
...
const uploadCanvasToStorage = async (canvas, folderName = 'maps') => {
...
// === БЕЗОПАСНАЯ МАТЕМАТИКА ТУМАНА (Для старых браузеров Safari/Mobile) ===
function getIntersectionFallback(ray, segment) {
...
function calculateVisibilityPolygonFallback(originX, originY, visionRadius, walls, mapWidth, mapHeight) {
    ...
    // === ИСПРАВЛЕНИЕ: Выпускаем 100 лучей во все стороны, чтобы свет стал круглым ===
...
// === КОД ВОРКЕРА ДЛЯ СОВРЕМЕННЫХ БРАУЗЕРОВ ===
...
export default function App() {
  ...
  const stateRefs = useRef({ authStep, userRole, currentUser, draggingTokenId });
 ...
 // === 2. МГНОВЕННЫЙ СИМУЛЯТОР БРОСКОВ (Для тех, кто отключил 3D) ===
    ...
    try {
        ...
        return players.some(p => {
            ...
            if (Array.isArray(p.fovPolygon) && p.fovPolygon.length > 2) {
                ...
                // Проверяем 4 крайние точки токена (если центр за стеной, но край торчит)
                const edges = [
  ...
  const startEditingProfile = () => {
    ...
    setTempProfile({ 
        ...
        attacks: activeCharacter.attacks || [], 
        ac: activeCharacter.ac || 10, 
        tokenColor: activeCharacter.stats?.tokenColor || '#3b82f6', 
        tokenFrame: activeCharacter.stats?.tokenFrame || 'solid', 
  ...
  const saveProfile = () => {
    ...
    delete updatedChar.tokenColor; 
    delete updatedChar.tokenFrame;
  ...
  const deleteCharacter = (charId) => {
    ...
    if (activeCharId === charId) {
        setActiveCharId(updatedCharacters[0].id);
    }
    setIsEditingProfile(false);
  };
 ...
 const createSession = (e) => {
  ...
  const addToken = (e) => {
    ...
    const newToken = { 
      ...
      x: centerX, 
      y: centerY, 
      size: sessionData.gridConfig?.enabled ? (sessionData.gridConfig.size || 60) : 60, 
      vision: newTokenType === 'player' ? (parseInt(newTokenVision) || 0) : 0, 
      hp: safeHp, 
      maxHp: safeHp, 
      ac: safeAc, 
      image: newTokenImage || null, 
      deathSaves: { successes: 0, failures: 0 }, 
      statuses: [], 
      isHidden: newTokenHidden,
  ...
  const addTokenFromPreset = (preset) => {
    ...
    const centerX = (containerRect.width / 2 - pan.x) / scale + jitterX; const centerY = (containerRect.height / 2 - pan.y) / scale + jitterY;
    
    const newToken = { 
      id: generateId(), 
      name: preset.name || 'Импортированный', 
      type: preset.type || 'monster', 
      color: preset.color || '#ef4444', 
      frame: 'solid', 
      x: centerX, 
      y: centerY, 
      size: sessionData.gridConfig?.enabled ? (sessionData.gridConfig.size || 60) : 60, 
      vision: parseInt(preset.vision || 0), 
      hp: Number(preset.hp) || 1, 
      maxHp: Number(preset.maxHp || preset.hp) || 1, 
      ac: Number(preset.ac) || 10, 
      image: preset.image || null, 
      deathSaves: { successes: 0, failures: 0 }, 
      statuses: [], 
      isHidden: preset.isHidden || false,
      stats: preset.stats || { str: 0, dex: 0, con: 0, int: 0, wis: 0, cha: 0 }, 
  ...
  const rollDeathSave = (tokenId) => {
    ...
    // Точечное обновление вместо перезаписи всего массива
    updateSession({ 
